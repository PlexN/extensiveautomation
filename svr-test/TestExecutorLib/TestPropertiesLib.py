...
def decodeListShared(parameter, sharedParameters=[]):
    ...
    for param in parametersList:
        ...
        if projectData is None:
            raise TestPropertiesException("ERR_PRO_016: list of shared parameter project not authorized" )
        ...
        if mainKeyFounded is None:
            raise TestPropertiesException("ERR_PRO_017: list of shared parameter not available: %s" % key )
        else:
...
def decodeShared(parameter, sharedParameters=[], projectName=None, separator=">", exception=True):
    ...
    if projectData is None:
        if exception:
            raise TestPropertiesException("ERR_PRO_016: shared parameter project not authorized: %s (Id=%s)" % (prjName, prjId) )
        else:
            return '' 
    else:
        # find the main key
        ...
        if mainKeyFounded is None:
            if exception:
                raise TestPropertiesException("ERR_PRO_017: shared parameter not available: %s" % mainKey )
            else:
                return ''
        else:
            # not second key
...
class Parameters:
    ...
    def shared(self, project, name, subname=''):
        ...
        @rtype: string, list, int
        """
        return decodeShared(parameter="::%s:%s" % (name,subname) , sharedParameters=self.__parametersShared, projectName=project)
